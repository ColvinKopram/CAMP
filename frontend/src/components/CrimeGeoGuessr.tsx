...
const CrimeGeoGuessr = () => {
  const [ws, setWs] = useState(null);
  const [gameState, setGameState] = useState('menu'); // menu, lobby, playing, round_end, game_end
  ...
  // Connect to WebSocket (SocketIO protocol)
  useEffect(() => {
    // CRITICAL: Prevent duplicate connections in React Strict Mode
    ...
    socket.onmessage = (event) => {
      try {
        ...
        // SocketIO uses a specific protocol - parse it
        if (message.startsWith('42')) {
          ...
          // Connection established - send connect to namespace
          ...
          // Connected to namespace - now truly ready
  ...
  const handleSocketEvent = (eventName, data) => {
    ...
    switch(eventName) {
      ...
      case 'round_start':
        ...
        // Clean up old map before creating new one
        ...
        setImageError(false); // Reset image error for new round
      ...
      case 'round_end':
        ...
        setTimeout(() => {
          if (mapInstanceRef.current && data.actual_location && window.L) {
            const L = window.L;
            try {
              L.marker([data.actual_location.latitude, data.actual_location.longitude], {
                icon: L.icon({
              ...
              }).addTo(mapInstanceRef.current).bindPopup('Actual Crime Location').openPopup();
            } catch (e) {
              console.log('Error adding actual location marker:', e);
            }
          }
        }, 100);
        break;
      ...
      case 'player_left':
        ...
        // If in game, show message but don't kick to menu
        if (gameState !== 'lobby') {
          setTimeout(() => {
            setError('');
          }, 5000);
      ...
      case 'room_closed':
        // Host left - kick everyone to menu
        setError(data.message);
        setTimeout(() => {
          backToMenu();
        }, 2000);
  ...
  const initMap = () => {
    ...
    map.on('click', (e) => {
      // Don't allow clicking if already guessed or not in playing state
  ...
  const submitGuess = () => {
    ...
    // Lock the marker - make it non-interactive
    if (markerRef.current && mapInstanceRef.current) {
      // Remove click handler from map to prevent further clicks
      mapInstanceRef.current.off('click');
      
      // Make marker non-draggable (in case it was draggable)
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-blue-900 via-purple-900 to-pink-900 flex items-center justify-center p-4">
        <div className="bg-white rounded-lg shadow-2xl p-8 max-w-md w-full">
          <h1 className="text-4xl font-bold text-center mb-2 text-gray-800">NYC Crime GeoGuessr</h1>
    ...
    return (
      <div className="h-screen flex flex-col bg-gray-900">
        ...
        <div className="flex-1 flex">
          {/* Left side - Two boxes stacked */}
          <div className="w-1/2 flex flex-col">
            {/* Top box - Street View Image */}
            ...
            {/* Bottom box - Empty for now */}
            <div className="h-1/2 bg-gray-800 flex items-center justify-center">
